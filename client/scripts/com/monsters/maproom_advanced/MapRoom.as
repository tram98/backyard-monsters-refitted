package com.monsters.maproom_advanced
{
   ...
   public class MapRoom implements IMapRoom
   {
      ...
      internal static function AcceptInvitation(param1:Boolean = false) : void
      {
         ...
         if(Boolean(_migrateThread) && _inviteBaseID != 0)
         {
            ...
            else
            {
               ...
               loadvars.push(["resources",JsonUtil.encode({
                  "r1":RESOURCECOST.Get(),
                  "r2":RESOURCECOST.Get(),
                  "r3":RESOURCECOST.Get(),
                  "r4":RESOURCECOST.Get()
      ...
      internal static function BookmarksSave() : void
      {
         ...
         var loadvars:Array = [["bookmarks",JsonUtil.encode(_bookmarkData)]];
         ...
         if(_monsterTransferInProgress)
         {
            if(targetCell._mine)
            {
               ...
               if(targetCell._monsters && _monsterSource && targetCell._monsterData.space.Get() > 0)
               {
                  ...
                  if(!targetCell.Check())
                  {
                     LOGGER.Log("err","BASE.Save:  transfer target Cell " + targetCell.X + "," + targetCell.Y + "does not check out before doing monster transfer!  " + JsonUtil.encode(targetCell._hpMonsterData));
                  }
                  if(!_monsterSource.Check())
                  {
                     LOGGER.Log("err","BASE.Save:  transfer source Cell " + _monsterSource.X + "," + _monsterSource.Y + "does not check out before doing monster transfer!  " + JsonUtil.encode(_monsterSource._hpMonsterData));
                  }
                  srcMonsterData = {
                  ...
                  transferVars = [["frombaseid",_monsterSource._baseID],["tobaseid",targetCell._baseID],["monsters",JsonUtil.encode([srcMonsterData,targetMonsterData])]];
